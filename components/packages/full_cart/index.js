...
const FullCart = ({
  ...
  const paymentOptions = ['visa', 'pp', 'mastercard'];
      ...
      <div className="acf-full-cart__inner">
        ...
        {count > 0 && (
          <div className="acf-checkout">
            ...
            <a className="acf-checkout__button" href={ACF_PATH_CHECKOUT}>
              Checkout
            </a>
            <div className="acf-checkout__accept">We Accept:</div>
            <ul className="acf-checkout__payment-options">
              {paymentOptions.map(paymentOption => (
                <li className={`acf-checkout__payment-${paymentOption}`} />
              ))}
            </ul>
          </div>
        )}
...
const FullCartWithStore = connect(
  mapStateToProps,
  mapDispatchToProps
)(FullCart);
